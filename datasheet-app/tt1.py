...
def extract_i2c_info(_chunks):
    """Enhanced I2C info extraction with caching"""
    i2c_addresses = []
    registers = {}
    voltage_info = []
    pin_info = []
    ...
    reg_patterns = [
        ...
        r'Table.*?Register.*?(0x[0-9a-fA-F]{2,4})[\s:]+([\w]+)',
    ]
    
    addr_patterns = [
        (r'(?:I2C|i2c)\s+(?:slave\s+)?address[\s:=]*(0x[0-9a-fA-F]{2})', 20),
        (r'(?:device|slave)\s+address[\s:=]*(0x[0-9a-fA-F]{2})', 15),
        (r'address[\s:=]*(0x[0-9a-fA-F]{2})', 5),
    ]
    
    for chunk in _chunks:
        text = chunk["text"]
        
        # I2C addresses with priority
            ...
            for match in reg_matches:
                if isinstance(match, tuple):
                    if match[0].startswith('0x'):
                        registers[match[1].upper()] = match[0]
                    else:
                        registers[match[0].upper()] = match[1]
...
with st.sidebar:
    ...
    st.caption("✓ Multi-chunk ranking")
    st.caption("✓ Cached extraction")
                    ...
                    try:
                        ...
                        if result["answer"] == "• Answer not found in datasheet.":
                            ...
                            if result["results"]:
                                ...
                                st.info(f"💡 Top match score ({top_score:.2f}) below threshold ({confidence:.2f}). Try lowering the confidence slider or rephrasing.")
                            else:
                                st.info("💡 No relevant chunks found. Try rephrasing your question with different keywords.")
                        else:
